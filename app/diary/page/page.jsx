...
export default function Page() {
  ...
  const { id, title, description } = pageData[pageData.length - 1] || {};
  ...
  useEffect(() => {
    const fetchOldText = async () => {
      if (user && id) {
        //console.log(title);
        try {
          const response = await axios.post(`/api/diary/page/${id}`);
          //console.log(response);
          if (response.data && response.data.text) {
            setText(response.data.text.split("\n"));
          }
        } catch (error) {
          console.error("Error fetching old text:", error);
        }
      }
    };
  ...
  const exportText = () => {
    const savedText =
      typeof window !== "undefined"
        ...
        : "";
    //console.log("asdf", title, description);
    const data = {
      title: title,
